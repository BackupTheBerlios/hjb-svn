...
/**
 * <code>DeleteConnectionFactoryGenerator</code> is the
 * <code>JMSCommandGenerator</code> that produces
 * {@link DeleteConnectionFactory}.
 * 
 ...
 * @author Tim Emiola
 */
public class DeleteConnectionFactoryGenerator extends
    ...
    protected void constructCommandFrom(HttpServletRequest request, HJBRoot root) {
        ...
        this.generatedCommand = new DeleteConnectionFactory(walker.findProvider(providerName),
                                                            factoryName);
    }
    ...
    private static final Pattern PATH_MATCHER = Pattern.compile("^/(\\w+)/(?!"
            + PathNaming.DESTINATION + ")(?!.*?" + PathNaming.CONNECTION
            + "-\\d+)(.+?)/?$");
}
