...
public class MapMessageByteArrayValueCopier extends MapMessageValueCopier {

    public MapMessageByteArrayValueCopier() {
        super(new ByteArrayCodec());
    }
    ...
    public boolean canBeEncoded(String name, Message message)
            throws HJBException {
        try {
            asAMapMessage(message).getBytes(name);
            return true;
        } catch (JMSException e) {
            return false;
