...
public class StreamMessageDoubleValueCopier extends StreamMessageValueCopier {

    public StreamMessageDoubleValueCopier(LinkedList valuesRead) {
        super(new DoubleCodec(), valuesRead);
    }
    ...
    public String getAsEncodedValue(String name, Message message)
        ...
        synchronized (values) {
            try {
                if (0 == values.size()) {
                    return encode(new Double(asAStreamMessage(message).readDouble()));
                } else {
