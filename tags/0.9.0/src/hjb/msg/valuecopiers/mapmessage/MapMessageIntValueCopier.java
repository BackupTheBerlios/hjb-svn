...
public class MapMessageIntValueCopier extends MapMessageValueCopier {

    public MapMessageIntValueCopier() {
        super(new IntCodec());
    }
    ...
    public void addToMessage(String name, String encodedValue, Message message)
            throws HJBException {
        try {
            asAMapMessage(message).setInt(name, decodeAsInteger(encodedValue));
    ...
    public boolean canBeEncoded(String name, Message message)
            throws HJBException {
        try {
            asAMapMessage(message).getInt(name);
            return true;
        } catch (JMSException e) {
            return false;
    ...
    public String getAsEncodedValue(String name, Message message)
            throws HJBException {
        try {
            return encode(new Integer(asAMapMessage(message).getInt(name)));
        } catch (JMSException e) {
