...
public class ContentWritingResponderTest extends MockObjectTestCase {
    ...
    public void testSendResponseHandlesSuccessCorrectly() throws Exception {
        ...
        mockResponse.expects(once()).method("setStatus").with(eq(HttpServletResponse.SC_OK));
        mockResponse.expects(once()).method("setContentType").with(eq(HJBServletConstants.HTTP_TEXT_PLAIN));
        mockResponse.expects(once()).method("getWriter").will(returnValue(new PrintWriter(testWriter)));
        HttpServletResponse testResponse = (HttpServletResponse) mockResponse.proxy();
