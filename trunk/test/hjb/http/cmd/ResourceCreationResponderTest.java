...
public class ResourceCreationResponderTest extends MockObjectTestCase {

    public void testResourceCreationResponderThrowsOnNullInputs() {
        try {
            Mock mockJMSCommand = mock(JMSCommand.class);
            JMSCommand testCommand = (JMSCommand) mockJMSCommand.proxy();
            new ResourceCreationResponder(testCommand, null);
            fail("Should have thrown and exception");
        } catch (IllegalArgumentException e) {}
        try {
            new ResourceCreationResponder(null, "");
            fail("Should have thrown and exception");
        } catch (IllegalArgumentException e) {}
    }
    ...
    public void testSendResponseHandlesFaultsCorrectly() throws Exception {
        ...
        mockJMSCommand.stubs().method("getStatusMessage").will(returnValue("test Status"));
        mockJMSCommand.expects(once()).method("getFault").will(returnValue(new HJBException("thrown as a Test")));

        JMSCommand testCommand = (JMSCommand) mockJMSCommand.proxy();

        Mock mockResponse = mock(HttpServletResponse.class);
        mockResponse.expects(once()).method("addHeader").with(eq(HJBServletConstants.HJB_STATUS_HEADER),
                                                              eq("test Status"));
        mockResponse.expects(once()).method("sendError").with(eq(HttpServletResponse.SC_INTERNAL_SERVER_ERROR),
                                                              eq("test Status"));
    ...
    public void testSendResponseHandlesSuccessCorrectly() throws Exception {
        ...
        mockResponse.expects(once()).method("setStatus").with(eq(HttpServletResponse.SC_CREATED));
        mockResponse.expects(once()).method("addHeader").with(eq(HJBServletConstants.HTTP_LOCATION),
                                                              eq("testLocation"));
        HttpServletResponse testResponse = (HttpServletResponse) mockResponse.proxy();
