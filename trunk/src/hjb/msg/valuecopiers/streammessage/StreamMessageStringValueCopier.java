...
public class StreamMessageStringValueCopier extends StreamMessageValueCopier {

    public StreamMessageStringValueCopier(LinkedList valuesRead) {
        super(new StringCodec(), valuesRead);
    }
    ...
    public String getAsEncodedValue(String name, Message message)
        ...
        synchronized (values) {
            try {
                if (0 == values.size()) {
                    String result = asAStreamMessage(message).readString();
                    ;
                    if (null == result) {
    ...
    private static final Logger LOG = Logger.getLogger(StreamMessageStringValueCopier.class);
}
