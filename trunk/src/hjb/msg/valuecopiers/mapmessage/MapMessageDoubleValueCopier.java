...
public class MapMessageDoubleValueCopier extends MapMessageValueCopier {

    public MapMessageDoubleValueCopier() {
    ...
    public boolean canBeEncoded(String name, Message message)
            throws HJBException {
        try {
            asAMapMessage(message).getDouble(name);
            return true;
        } catch (JMSException e) {
            return false;
    ...
    public String getAsEncodedValue(String name, Message message)
            throws HJBException {
        try {
            return encode(new Double(asAMapMessage(message).getDouble(name)));
        } catch (JMSException e) {
