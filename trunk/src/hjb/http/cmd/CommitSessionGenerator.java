 ...
 * that produces {@link CommitSession}.
 * 
 * @see hjb.http.cmd.JMSCommandGenerator
 * @see hjb.jms.cmd.JMSCommand
 * @author Tim Emiola
 */
public class CommitSessionGenerator extends PatternMatchingCommandGenerator {
    ...
    private static final Pattern PATH_MATCHER = Pattern.compile("^/(\\w+)/([^/]+)/connection-(\\d+)/session-(\\d+)/commit$");
}
